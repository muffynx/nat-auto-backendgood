...
# ────────────────────────────────────────────────
#   Agent Version Check (public — no auth needed)
# ────────────────────────────────────────────────
@app.route('/api/agent/version', methods=['GET'])
def agent_version():
    """
    Agent เรียก endpoint นี้ตอน startup เพื่อเช็ค version
    Query param: current=1.0.0
    """
    current = request.args.get('current', '0.0.0')
    is_outdated = current != AGENT_LATEST_VERSION
    return jsonify({
        ...
        'message': (
            f'⚠️  New version {AGENT_LATEST_VERSION} is available! Please update your Agent.'
            if is_outdated else 'Up to date'
        )
    })
...
def convert_config_api():
    ...
    target_type = None
    log_content = None
    sections = None  # ✅ รองรับ filter sections

    # CASE 1: Excel Upload
    if request.content_type and 'multipart/form-data' in request.content_type:
        source_type = request.form.get('source_type')
        target_type = request.form.get('target_type')
        sections_raw = request.form.get('sections')  # comma-separated string
        if sections_raw:
    ...
    # CASE 2: Text JSON
        ...
        sections = data.get('sections')  # list or None
    ...
    try:
        ...
        result_config = converter.process(sections=sections)
    ...
    try:
        ...
        with zipfile.ZipFile(memory_file, 'w', zipfile.ZIP_DEFLATED) as zf:
            for file in uploaded_files:
                ...
                # 2. Convert Config (พร้อม sections filter)
                converter = ConfigConverter(source_type, target_type, log_content)
                result_config = converter.process(sections=sections)
